...
(meditations
  ...
  (= "Rich Hickey aka The Clojurer aka Go Time aka Lambda Guru"
     (let [[first-name last-name & aliases]
       ...
       (clojure.string/join " " (concat [first-name last-name] (interleave (repeat "aka") aliases)))))
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     ((fn [full-name {:keys [street-address city state]}]
        (clojure.string/join ", " [(clojure.string/join " " full-name) street-address city state])) ["Test" "Testerson"] test-address)))
